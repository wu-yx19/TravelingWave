...
l_mean_temp = np.linspace(rf_range[0],rf_range[1],scale[0])
...
for i in range(scale[0]):
    ...
    for j in range(scale[1]):
        ...
        if i + 1 < scale[0]:
            temp = build_I_syn(
                all_blocks[i][j].inh.group, all_blocks[i + 1][j].exc.group, S_w0 * nA, S_wv * nA, S_cp, S_delay)
            net.add(temp)
            syn['lg_f'] = temp
        if j + 1 < scale[1]:
            temp = build_I_syn(
                all_blocks[i][j].inh.group, all_blocks[i][j + 1].exc.group, S_w0 * nA, S_wv * nA, S_cp, S_delay)
            net.add(temp)
            syn['ts_f'] = temp
        if i > 0:
            temp = build_I_syn(
...
for i in range(scale[0]):
    for j in range(scale[1]):
        ...
        block.save_spike(name, folder)   

print('Simulation End')
        
block.plot_spike(name, folder)
end = time.time()
print(end - start)
print('Saved!')
